...
void OpenCV_object_detect2(unsigned char* srcBuf, int iw, int ih, unsigned char* outBuf, int nw, int nh) {
	...
	int g_y , g_x , r_y , r_x , y_x , y_y = 0;

	for (int j = 0; j < ih; j++) {

		for (int i = 0; i < iw; i++) {

			if (srcRGB.at<cv::Vec3b>(j,i)[0] <= 30 && srcRGB.at<cv::Vec3b>(j, i)[1] <= 150 && srcRGB.at<cv::Vec3b>(j, i)[1] >= 60 && srcRGB.at<cv::Vec3b>(j, i)[2] <= 30){
			...
			if (srcRGB.at<cv::Vec3b>(j, i)[0] <= 30 && srcRGB.at<cv::Vec3b>(j, i)[1] <= 30 && srcRGB.at<cv::Vec3b>(j, i)[2] >= 60 && srcRGB.at<cv::Vec3b>(j, i)[2] <= 150) {
			...
			if (srcRGB.at<cv::Vec3b>(j, i)[0] <= 30 && srcRGB.at<cv::Vec3b>(j, i)[1] >= 60 && srcRGB.at<cv::Vec3b>(j, i)[1] <= 150 && srcRGB.at<cv::Vec3b>(j, i)[2] >= 60 && srcRGB.at<cv::Vec3b>(j, i)[2] <= 150) {
	...
	y_x = yellowsum_y / yellowcnt;

	y_y = yellowsum_x / yellowcnt;
